...
export function computeLayout(cfg, W, H) {
    ...
    var mainFills   = cfg.mainFills   || null;
    var subFills    = cfg.subFills    || null;
    ...
    if (sections) {
        /* Multi-section: sections stacked vertically, per-section independent netz.
           EVERY section is exactly H (viewport height) tall.
           Content placed at secIdx*H + SQ so scroll boundary secIdx*H aligns. */
        sectionContents = [];
        var globalMainIdx = 0;
        ...
        for (var secIdx = 0; secIdx < sections.length; secIdx++) {
            ...
            /* Section domain: uniform H per section */
            var domainTop = secIdx * H;
            var domainBot = (secIdx + 1) * H;
            var curY = domainTop + SQ;  /* content starts SQ below section top */

            sectionRanges.push({ startMain: globalMainIdx, endMain: globalMainIdx + nM });

            /* Place mains: landscape → side-by-side, portrait → stacked */
            var secSlotW = isLand ? Math.round((innerW - GAP * Math.max(nM - 1, 0)) / nM) : innerW;
            ...
            for (var smi = 0; smi < nM; smi++) {
                ...
                var isFill = mainFills && mainFills[gmi];
                var maxMW = (nSubsS > 0) ? Math.round(availW / 1.5) : availW;
                var mWS, mHS;
                if (isFill) {
                    /* Fill mode: skip contain-fit, take full available slot */
                    mWS = maxMW; mHS = innerH;
                } else if (maxMW / innerH > mAspS) {
                ...
                /* Scale down if main + sub doesn't fit in slot (skip for fill nodes) */
                if (!isFill && isLand && (mWS + sWS) > secSlotW) {
                ...
                var anchorX = isLand
                    ? ((smi < nM - 1) ? SQ + smi * (secSlotW + GAP) + secSlotW : W - SQ)
                    : W - SQ;
                /* Sub anchor Y = content top of this section (curY), not absolute */
                var groupSubs = _placeSubs(mr, my, mb, anchorX, curY,
        ...
        totalH = sections.length * H;

    } else if (isLand) {
        totalH = H;
        var sectionW = Math.round((innerW - GAP * Math.max(mainCount - 1, 0)) / mainCount);
        ...
        for (var gi = 0; gi < mainCount; gi++) {
            ...
            var isFillL = mainFills && mainFills[gi];
            var nSubsL = subsPerMain[gi] || 0;
            var mH, mW, sW;
            if (isFillL) {
                /* Fill mode: take full slot, reserve sub space if needed */
                mW = (nSubsL > 0) ? Math.round(sectionW / 1.5) : sectionW;
                mH = innerH;
                sW = (nSubsL > 0) ? Math.round(mW / 2) : 0;
            } else {
                mH = innerH;
                mW = Math.round(mH * mAsp);
                sW = Math.round(mW / 2);
            }
            ...
            /* Scale down if main + sub doesn't fit horizontally (skip for fill nodes) */
            if (!isFillL && mW + sW > sectionW) {
            ...
            var secLeft = SQ + gi * (sectionW + GAP);
            ...
            var anchorX = (gi < mainCount - 1) ? secLeft + sectionW : W - SQ;
        ...
        for (var gi2 = 0; gi2 < mainCount; gi2++) {
            ...
            var isFillP = mainFills && mainFills[gi2];
            var maxWP = (nSubsP > 0) ? Math.round(innerW / 1.5) : innerW;
            var mWP, mHP;
            if (isFillP) {
                /* Fill mode: skip contain-fit, take full available slot */
                mWP = maxWP; mHP = innerH;
            } else if (maxWP / innerH > mAspP) {
            ...
            curY += innerH + ((gi2 < mainCount - 1) ? GAP : 0);
        }
    ...
    if (sectionContents && sectionContents.length > 1) {
        /* Per-section independent netz grids.
           Each section gets virtual nav-quad blockers at its viewport corners
           so netz carves out space for the fixed nav quads. */
        var allNetzCells = [];
        for (var ni = 0; ni < sectionContents.length; ni++) {
            var sc = sectionContents[ni];
            var secTop = sc.domainTop;
            var secBot = sc.domainBot;
            var blockers = [
                { x: 0,      y: secTop,        w: SQ, h: SQ },  /* TL */
                { x: W - SQ, y: secTop,        w: SQ, h: SQ },  /* TR */
                { x: 0,      y: secBot - SQ,   w: SQ, h: SQ },  /* BL */
                { x: W - SQ, y: secBot - SQ,   w: SQ, h: SQ }   /* BR */
            ];
...
function _placeSubs(mainR, mainT, mainB, anchorX, anchorY, subW, subH, count, availH, proj, projPts, cc, W, H, b2bFlags, subAspects_i) {
    ...
    var GAP = LAYOUT_CONST.GAP || 0;
    var subStartX = mainR + GAP;            /* horizontal gap: main → subs */
    var totalSubW = anchorX - subStartX;    /* total available width for subs */
    if (totalSubW < 10) totalSubW = subW;

    /* ── Step 1: Group subs into rows via b2b flags ── */
    ...
    /* ── Step 2: Measure naive stacked height ── */
    function _getSubH(idx) {
        return Array.isArray(subH) ? (subH[idx] || Math.round(subW * 0.75)) : subH;
    }
    function _getSubAsp(idx) {
        return (subAspects_i && subAspects_i[idx] != null) ? subAspects_i[idx] : (subW / _getSubH(idx));
    }
    ...
    for (var ri0 = 0; ri0 < rows.length; ri0++) {
        var row0 = rows[ri0];
        if (row0.length === 1) {
            naiveH += _getSubH(row0[0]);
        } else {
            var colW0 = Math.floor(totalSubW / row0.length);
            var rh0 = 0;
            for (var ci0 = 0; ci0 < row0.length; ci0++) {
                var h0 = Math.round(colW0 / _getSubAsp(row0[ci0]));
                if (h0 > rh0) rh0 = h0;
            }
            naiveH += rh0;
        }
    }
    naiveH += GAP * Math.max(rows.length - 1, 0);

    /* ── Step 3: If subs overflow availH → re-layout as multi-column grid ── */
    if (naiveH > availH && count > 1) {
        /* Determine column count: 2 cols for 2-7 subs, 3 cols for 8+ */
        var nCols = (count >= 8) ? 3 : 2;
        var cellW = Math.floor((totalSubW - GAP * (nCols - 1)) / nCols);

        /* Compute cell heights from aspect ratios */
        var cellHArr = [];
        for (var gi = 0; gi < count; gi++) {
            var asp_g = _getSubAsp(gi);
            cellHArr.push(Math.round(cellW / asp_g));
        }

        /* Distribute subs into rows of nCols, compute per-row height */
        var nRows = Math.ceil(count / nCols);

        /* Check total grid height; if still overflows, scale down all cells */
        var gridH = 0;
        for (var rr = 0; rr < nRows; rr++) {
            var maxInRow = 0;
            for (var cc2 = 0; cc2 < nCols; cc2++) {
                var si2 = rr * nCols + cc2;
                if (si2 < count && cellHArr[si2] > maxInRow) maxInRow = cellHArr[si2];
            }
            gridH += maxInRow + (rr < nRows - 1 ? GAP : 0);
        }

        if (gridH > availH) {
            var scale = availH / gridH;
            cellW = Math.floor(cellW * scale);
            for (var si3 = 0; si3 < count; si3++) {
                cellHArr[si3] = Math.round(cellW / _getSubAsp(si3));
            }
        }
        ...
        for (var rr2 = 0; rr2 < nRows; rr2++) {
            ...
            for (var cc3 = 0; cc3 < nCols; cc3++) {
                var si4 = rr2 * nCols + cc3;
                if (si4 >= count) continue;
                var cH = cellHArr[si4];
                if (cH > maxRowH) maxRowH = cH;
            }
            for (var cc4 = 0; cc4 < nCols; cc4++) {
                var si5 = rr2 * nCols + cc4;
                if (si5 >= count) continue;
                var cH2 = cellHArr[si5];
                var sx_g = subStartX + cc4 * (cellW + GAP);
                var sR_g = sx_g + cellW;
                var sy_g = curY_g;
                var sB_g = sy_g + cH2;
                g[si5] = { x: sx_g, y: sy_g, w: cellW, h: cH2, r: sR_g, b: sB_g };
                cc.push([sx_g,sy_g],[sR_g,sy_g],[sR_g,sB_g],[sx_g,sB_g]);
                proj.push({ from:[sx_g,sy_g], to:[sx_g,0] }, { from:[sx_g,sB_g], to:[sx_g,H] },
                          { from:[sR_g,sy_g], to:[W,sy_g] }, { from:[sR_g,sB_g], to:[W,sB_g] });
                projPts.push([sx_g,0],[sx_g,H],[W,sy_g],[W,sB_g]);
            }
            curY_g += maxRowH + GAP;
        }
        return g;
    ...
    /* ── Step 4: Normal stacked layout (fits within availH) ── */
    /* If total height slightly exceeds, scale everything proportionally */
    var scaleFactor = 1;
    if (naiveH > availH) {
        scaleFactor = availH / naiveH;
    }
    ...
    for (var ri = 0; ri < rows.length; ri++) {
        ...
        if (row.length === 1) {
            ...
            var idx = row[0];
            var sH = Math.round(_getSubH(idx) * scaleFactor);
            var sW_s = (scaleFactor < 1) ? Math.round(subW * scaleFactor) : subW;
            var sx = subStartX, sR = sx + sW_s, sy = curY, sB = sy + sH;
            g[idx] = { x:sx, y:sy, w:sW_s, h:sH, r:sR, b:sB };
            ...
            curY = sB + ((ri < rows.length - 1) ? GAP : 0);
        } else {
            /* Multiple b2b subs → place side-by-side in a row */
            var colW = Math.floor(totalSubW / row.length);
            var scaledColW = (scaleFactor < 1) ? Math.round(colW * scaleFactor) : colW;
            /* Row height = max of all sub heights in this row (scaled to colW) */
            var rowH = 0;
            for (var ci = 0; ci < row.length; ci++) {
                var sidx = row[ci];
                var h = Math.round(scaledColW / _getSubAsp(sidx));
                if (h > rowH) rowH = h;
            }
            for (var ci2 = 0; ci2 < row.length; ci2++) {
                var sidx2 = row[ci2];
                var sx2 = subStartX + ci2 * colW;
                var sR2 = sx2 + scaledColW;
                var sy2 = curY, sB2 = sy2 + rowH;
                g[sidx2] = { x:sx2, y:sy2, w:scaledColW, h:rowH, r:sR2, b:sB2 };
            ...
            curY += rowH + ((ri < rows.length - 1) ? GAP : 0);
        }
    }
    return g;
}
